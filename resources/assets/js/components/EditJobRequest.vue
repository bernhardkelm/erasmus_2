<template>
    <section class="dashboard__requests">
        <loading></loading>
        <div v-if="!loading">
          <h3 class="dashboard__title">Edit Job Request</h3>
          <div class="clearfix"></div>
          
          <div class="field">
            <label for="title">Titel</label>
            <input type="text" id="title" v-model="jobRequest.title">
          </div>
          <div class="field">
            <label for="body">Description</label>
            <textarea id="body" v-model="jobRequest.body"></textarea>
          </div>
          ...
          <div class="button-group">
            ...
            <router-link to="/dashboard/jobrequests" class="button is-outline">Back</router-link>
            <button 
...
<script>
    ...
    export default {
        ...
        mounted: function() {
            ...
            this.$store.dispatch('FETCH_REQUEST', this.$route.params.id)
              .then((response) => {
                this.jobRequest = JSON.parse(JSON.stringify(response));
              });
        },
        methods: {
            /**
            ...
            destroy() {
              ...
              this.$store.dispatch('DESTROY_REQUEST', this.jobRequest.id)
                .then(() => {
